...
import { createKeys } from '$lib/ui/table/context.js';
...
export function ensureFieldsWithFilterAreVisibles(
	...
	const fieldsVisible = jsonParse<string[]>(
		url.searchParams.get(KEY_FIELDS_VISIBLE),
		[]
	);
	const fieldsHidden = jsonParse<string[]>(
		url.searchParams.get(KEY_FIELDS_HIDDEN),
		[]
	);
	...
	const paramsUpdated =
		_fieldsVisible.length !== fieldsVisible.length ||
		_fieldsHidden.length !== fieldsHidden.length;
